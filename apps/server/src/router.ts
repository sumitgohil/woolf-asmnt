...
export const appRouter = router({
  ...
  checkAI: publicProcedure.query(() => {
    return {
      ...
      authToken: process.env.AUTHORIZATION_TOKEN ? "configured" : "not configured",
      timestamp: new Date().toISOString(),
    };
  }),
  ...
  analyzePDFs: publicProcedure
    .input(z.object({
      jobDescriptionBase64: z.string().min(1, "Job description PDF is required"),
      cvBase64: z.string().min(1, "CV PDF is required"),
    }))
    .mutation(async ({ input }) => {
      try {
        const jobDescriptionBuffer = Buffer.from(input.jobDescriptionBase64, "base64");
        const cvBuffer = Buffer.from(input.cvBase64, "base64");

        console.log("Extracting text from job description PDF...");
        const jobDescriptionText = await PDFService.extractText(jobDescriptionBuffer);
        ...
        const result = await AIService.analyzeCVMatch(jobDescriptionText, cvText);
        ...
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: error instanceof Error ? error.message : "PDF analysis failed"
        });
      }
    }),
