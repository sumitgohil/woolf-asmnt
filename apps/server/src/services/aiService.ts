...
export class AIService {
  ...
  static async analyzeCVMatch(jobDescription: string, cv: string): Promise<AnalysisResult> {
    ...
    const prompt = ANALYSIS_PROMPTS.USER_TEMPLATE
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 401) {
          throw new Error("Authentication failed. Please check your AUTHORIZATION_TOKEN.");
        }
        if (error.response?.status === 403) {
          throw new Error("Permission denied. Please verify your API key permissions.");
        }
  ...
  private static async callVertexAI(request: GenerateContentRequest): Promise<GenerateContentResponse> {
  ...
  private static parseAnalysisResponse(response: GenerateContentResponse): AnalysisResult {
    ...
    try {
      ...
      if (!Array.isArray(parsed.strengths) || !Array.isArray(parsed.weaknesses) || 
          !Array.isArray(parsed.recommendations) || typeof parsed.alignment !== "number" ||
